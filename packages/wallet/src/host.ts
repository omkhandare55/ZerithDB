...
export class WalletHost {
  ...
  private async handleRequest(request: WalletRequest, origin: string, source: Window): Promise<void> {
    try {
      let result: any;
      ...
      switch (request.type) {
        ...
        case "WALLET_SIGN":
          result = await this.auth.sign(request.payload);
          break;
  ...
  private async handlePickFile(payload: any): Promise<any> {
    ...
    return new Promise((resolve) => {
      ...
      setTimeout(() => {
        resolve({
          collection: payload?.collection ?? "shared_files",
          id: "picked-file-123",
          data: {
